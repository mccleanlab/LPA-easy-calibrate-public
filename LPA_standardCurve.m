% SUMMARY:
% This script generates a standard curve for a calibrated light plate
...
%% Last revised on November 9, 2018
...
sensorUnits = 'uW/cm^2';
targetLightOutput = [0 33.7 123.7 310.7]';
...
for i = 1:numFiles
    ...
    if max(idx) > numWellsMeasured
        disp(['Warning! More than ' num2str(numWellsMeasured) ' wells detected' newline 'Try changing the ampthresh parameter']);
    elseif max(idx) < numWellsMeasured
        disp(['Warning! Fewer than ' num2str(numWellsMeasured) ' wells detected' newline 'Try changing the ampthresh parameter']);
    end
    ...
    for j = 1:numWellsMeasured
        ...
        xlabel('Samples')
        ylabel(['Irradiance' newline '(W/cm^2)'])
    end
...
%% Generate and display the standard curve equation
...
options.Lower = [-Inf 0];
options.Upper = [Inf 0];
[f, g] = fit(measurementValues, u,'poly1',options);
x = 0:1:max(measurementValues);
yfit = f.p1*x + f.p2;
doseEqn = ['Irradiance = ' num2str(1E6*f.p1) '*IRIS'];
...
if isempty(inputIRIS(inputIRIS>4095))~=1
    disp('Warning! A calculated Iris value exceeds 4095.')
end

% Plot standard curve equation versus fitted data
figure('Name', 'LPA standard curve')
errorbar(measurementValues,u*1E6,sd*1E6,'ro','MarkerSize',2);
hold on;
h = plot(x,yfit*1E6);
title(['LPA standard curve' newline '(R squared = ' num2str(g.rsquare) ')']); xlabel('IRIS values (AU)'); ylabel(['Irradiance (' sensorUnits ')'])
